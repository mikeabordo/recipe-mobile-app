  ...
  return (
    <ScrollView style={globalStyles.container} showsVerticalScrollIndicator={false}>
      ...
      >
        ...
        <Text className="text-slate-400 text-sm font-medium">Back</Text>
      </TouchableOpacity>
      ...
      <View className="mb-4">
        <View className="flex-row justify-between items-start mb-2">
          ...
          <View className="flex-row items-center gap-1 bg-amber-500/20 border border-amber-500/40 px-3 py-1.5 rounded-full">
            ...
            <Text className="text-amber-400 text-sm font-bold">{recipe.rating}</Text>
          </View>
        </View>
        <Text className="text-slate-400 text-sm leading-relaxed">{recipe.description}</Text>
      </View>
      ...
      <View className="flex-row justify-between mb-6">
        <View className="flex-1 items-center bg-slate-800/80 border border-slate-700/80 rounded-2xl py-3 mx-1">
          ...
          <Text className="text-white text-sm font-bold mt-1">{recipe.duration}</Text>
        ...
        <View className="flex-1 items-center bg-slate-800/80 border border-slate-700/80 rounded-2xl py-3 mx-1">
          ...
          <Text className="text-white text-sm font-bold mt-1">{recipe.calories}</Text>
      ...
      <View className="flex-row gap-2 mb-6">
        <View className="bg-teal-100/90 px-3 py-1 rounded-full">
          <Text className="text-teal-900 text-xs font-medium">{recipe.category}</Text>
        </View>
        <View className="bg-indigo-100/90 px-3 py-1 rounded-full">
          <Text className="text-indigo-900 text-xs font-medium">{recipe.cuisine}</Text>
        </View>
      </View>
          ...
          {recipe.steps.map((step, index) => (
            ...
            >
              <View className="w-7 h-7 rounded-full bg-teal-500/20 border border-teal-500/40 items-center justify-center shrink-0">
                <Text className="text-teal-400 text-xs font-bold">{index + 1}</Text>
              </View>
              <Text className="text-slate-300 text-sm leading-relaxed flex-1">{step}</Text>
            </View>
          ))}
        </View>
      </View>
